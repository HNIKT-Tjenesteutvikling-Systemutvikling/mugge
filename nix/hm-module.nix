...
# which re-attaches to the live session. Detach again with Ctrl-\; `/quit`
# stops the background service (it exits 0, so systemd does not restart it).
self:
{
  config,
  lib,
...
let
  ...
  attach = pkgs.writeShellScriptBin "mugge" ''
    ...
    echo "type /quit to stop the background service entirely."
